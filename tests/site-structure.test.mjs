...
test("homepage is a two-experience destination before menu preview", () => {
  ...
  assert.match(home, /createEventsJsonLd/);
...
test("SEO metadata is unique per route with canonical URLs and structured data", () => {
  ...
  assert.match(seo, /createEventsJsonLd/);
  assert.match(seo, /schemaEventDetails/);
  assert.match(seo, /createEventJsonLd/);
  assert.match(seo, /nextWeeklyDate/);
  assert.match(seo, /nextMonthlyDate/);
  assert.match(seo, /formatIndiaDateTime/);
  assert.match(seo, /toIndiaWallClock/);
  assert.match(seo, /indiaOffsetMinutes = 330/);
  assert.match(seo, /"Friday Acoustic Night":/);
  assert.match(seo, /"Weekend Snooker Tournament"/);
  assert.match(seo, /"PS5 FIFA Challenge"/);
  assert.match(seo, /"Couples Evening"/);
  assert.match(seo, /"Rooftop Chill Session"/);
  assert.match(seo, /Live acoustic music on the rooftop at Flames of Arabia every Friday evening/);
  assert.match(seo, /startDate: formatIndiaDateTime\(startDate\)/);
  assert.match(seo, /endDate: formatIndiaDateTime\(endDate\)/);
  assert.match(seo, /weekOfMonth: 2/);
  assert.match(seo, /repeatFrequency: event\.date === "Monthly" \? "P1M" : "P1W"/);
  assert.match(seo, /"Every Friday": "https:\/\/schema\.org\/Friday"/);
  assert.match(seo, /byDay: byDay\[event\.date\] \?\? schemaDayUrls\[detail\.weekday\]/);
  assert.match(seo, /endTime: "23:59"/);
  assert.match(seo, /performerName: "Live Acoustic Artist"/);
  assert.match(seo, /performerName: "Flames 147 Players"/);
  assert.match(seo, /performerName: "Flames 147 Gaming Guests"/);
  assert.match(seo, /validFrom: "2026-01-01T00:00:00\+05:30"/);
  assert.match(seo, /name: "Flames of Arabia"/);
  assert.match(seo, /name: "Flames 147"/);
